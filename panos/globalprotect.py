...
class Roles(VersionedPanObject):
    ...
    def _setup(self):
        ...
        params.append(
            VersionedParamPath(
                "disconnection_on_idle_unit",
                path="disconnection-on-idle/{disconnection_on_idle_unit}",
                values=("days", "hours", "minutes"),
            )
        )
        params.append(
            VersionedParamPath(
                "disconnection_on_idle",
                vartype="int",
                path="disconnection-on-idle/{disconnection_on_idle_unit}",
            )
        )
    ...
    def _setup(self):
        ...
        params.append(
            VersionedParamPath(
                ...
                vartype="yesno",
                path="retrieve_framed_ip_address",
            )
        )
        params.append(
            VersionedParamPath(
                "no_direct_access_to_local_networks",
                vartype="yseno",
                path="no_direct_access_to_local_networks",
            )
        )
